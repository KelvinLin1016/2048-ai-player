...
class Qlearning:
	def __init__(self,board,learning_rate=0.01,reward_decay=0.9,e_greedy=0.9):
		self.actions=['a','s','d','w']
	...
	def choose_action(self,board):
		self.check_state_exist(board)
		if np.random.uniform()<self.epsilon:
			state_action=self.q_table[board,:]#self.q_table.ix[state,:]
	...
	def learn(self,s,a,r,s_):
		...
		q_predict = self.q_table[s,a]#.ix[s, a]
		if s_ != 'terminal':
...
class environment(object):
	def __init__(self):
		self.action_space=['a','s','d','w']
		self.n_actions=len(self.action_space)
		
	def step(self,action):
		
		actual_board.shift(action)
		board_next=actual_board.computer_move()
		s_=board_next
		if len(get_empty_spots(s_)) is 0:###game ends
			done=False
...
def encode_board(board):  ######integer board
...
def spawn_tile(board):
    ...
    if len(empty_spots) is 0:###game ends
        return (board, False)
...
def get_best_move(board):#####change
    ...
    for move_dir in 'awsd':
        ...
        if test_score > best_score:
            ...
            best_move = move_dir
    return best_move,reward


def smart_move(dt):    #####change
    enc_board = encode_board(actual_board)
    print(actual_board.score)
    b_move_r=get_best_move(enc_board)
    reward=b_move_r[1]
    end=actual_board.shift(b_move_r[0])
...
def simulate():
    ...
    for i in range(0, 10000):
        b_move_r=get_best_move(b)
        b, moved, points = smart_shift(b, b_move_r[0])
        score += points
        b, success = spawn_tile(b)
...
def simulate_show():
    ...
    for i in range(0, 10000):
        b_move_r=get_best_move(b)
        pyglet.clock.schedule_interval(b_move_r[0], 1/120)
        pyglet.app.run()
        b, moved, points = smart_shift(b, b_move_r[0])
