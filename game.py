...
from Reinforcement_AI import *#smart_move, set_board,get_best_move,Qlearning,environment
...
def start(graphic=True, ai_solve=True):
    global board
    board = Board(graphic)
...
start()
def start_player(graphic=True, ai_solve=False):
	global board
	board=Board(graphic)
	pyglet.app.run()

def start_AI(graphic=True, ai_solve=True):
    global board
    #env=environment()
    q=Qlearning(Board)
    for episode in range(100):
        repeat=True
        board = Board(graphic)#initalize board state
        #RUN=Qlearning(board,actions=list(range(env.n_actions)))
        while repeat==True:
            #print(board)
            set_board(board)
            if graphic:
            # for _ in range(0, 20):
            #     smart_move(0)
                action=q.choose_action(board)
                #print(action)
                #state,reward,done=env.step(action)
                pyglet.clock.schedule_interval(smart_move, 1/120)
                pyglet.app.run()
                board = Board(graphic)
                print('asdfasdfasdfasdfasdfasdfaf')
                repeat=False
            else:
                for i in range(0, 100):
                    smart_move(0)
                    if board.lost:
                        break
            board.show()
            print("Score:", board.score)#reward
            #if done:
             #   break
...
def learn_play():
	...
	for episode in range(5):
		while done==True:
			board=Board(True)
			print(board.board)
			#board_now=board.board()
			action_now=q.choose_action(board)
			(board_next,r,done)=env.step(action_now)		
			#actual_board.shift(action_now)
...
def worker(i):
    ...
    if i==test_cases+1:
        pyglet.clock.schedule_interval(simulate_show, 1/120)
        pyglet.app.run()
        board, s, m=simulate_show()
    else:  
        board, s, m = simulate()
    avg_score += s
    avg_moves += m
